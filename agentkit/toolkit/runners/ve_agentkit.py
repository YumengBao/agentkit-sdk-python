...
from agentkit.toolkit.config import CommonConfig, AUTO_CREATE_VE, is_valid_config, is_invalid_config
...
class VeAgentkitRunnerConfig(AutoSerializableMixin):
    ...
    runtime_id: str = field(default=AUTO_CREATE_VE, metadata={"description": "Runtime ID; 'Auto' means auto-create"})
    runtime_name: str = field(default=AUTO_CREATE_VE, metadata={"description": "Runtime name; 'Auto' means auto-generate"})
    runtime_role_name: str = field(default=AUTO_CREATE_VE, metadata={"description": "Runtime role name; 'Auto' means auto-create"})
    runtime_apikey: str = field(default="", metadata={"description": "Runtime API key"})
    runtime_apikey_name: str = field(default=AUTO_CREATE_VE, metadata={"description": "Runtime API key name; 'Auto' means auto-generate"})
    runtime_endpoint: str = field(default="", metadata={"description": "Runtime endpoint URL"})
    runtime_envs: Dict[str, str] = field(default_factory=dict, metadata={"description": "Runtime environment variables"})
...
class VeAgentkitRuntimeRunner(Runner):
    ...
    def destroy(self, config: VeAgentkitRunnerConfig) -> bool:
        ...
        try:
            ...
            self.agentkit_runtime_client.delete_runtime(runtime_types.DeleteRuntimeRequest(runtime_id=runner_config.runtime_id))
            self.reporter.success(f"Runtime destroyed successfully: {runner_config.runtime_id}")
            return True
            
        except Exception as e:
            if "InvalidAgentKitRuntime.NotFound" in str(e):
                self.reporter.info(f"Runtime not found or already destroyed: {runner_config.runtime_id}")
                return True
    ...
    def status(self, config: VeAgentkitRunnerConfig) -> StatusResult:
        ...
        try:
            ...
            return StatusResult(
                ...
                health="healthy" if ping_status else "unhealthy" if ping_status is False else None,
                metadata={
    ...
    def invoke(self, config: VeAgentkitRunnerConfig, payload: Dict[str, Any], headers: Optional[Dict[str, str]] = None, stream: Optional[bool] = None) -> InvokeResult:
        ...
        try:
            ...
            if success:
                ...
                return InvokeResult(
                    success=False,
                    error=error_msg,
                    error_code=ErrorCode.INVOKE_FAILED
                )
    ...
    def _prepare_runtime_config(self, config: VeAgentkitRunnerConfig) -> bool:
        ...
        try:
            ...
            if config.runtime_role_name == AUTO_CREATE_VE or not config.runtime_role_name:
                ...
                self.reporter.success(f"Generated role name: {config.runtime_role_name}")
            ...
            if config.runtime_apikey_name == AUTO_CREATE_VE or not config.runtime_apikey_name:
                ...
                self.reporter.success(f"Generated API key name: {config.runtime_apikey_name}")
    ...
    def _create_new_runtime(self, config: VeAgentkitRunnerConfig) -> DeployResult:
        ...
        try:
            ...
            envs = [runtime_types.EnvsItemForCreateRuntime(key=k, value=v) for k, v in config.runtime_envs.items()]
            ...
            self.reporter.info("💡 Tip: Runtime is initializing. Please wait patiently and do not interrupt.")
            ...
            if not success:
                self.reporter.warning(f"Runtime failed to initialize: {config.runtime_id}")
                ...
                should_cleanup = self.reporter.confirm("Do you want to clean up the failed Runtime?", default=False)
                
                if should_cleanup:
                    self.reporter.info(f"Cleaning up failed Runtime: {config.runtime_id}")
                    try:
                        self.agentkit_runtime_client.delete_runtime(runtime_types.DeleteRuntimeRequest(runtime_id=config.runtime_id))
                    ...
                    self.reporter.info(f"Cleanup skipped, Runtime retained: {config.runtime_id}")
    ...
    def _download_and_show_runtime_failed_logs(
        ...
        """
        if not runtime or not hasattr(runtime, 'failed_log_file_url') or not runtime.failed_log_file_url:
        ...
        try:
            ...
            self.reporter.show_logs(
                title="Runtime Failure Logs (First 50 lines)",
                lines=lines,
                max_lines=50
            )
    ...
    def _wait_for_runtime_status_multiple(
        ...
        with self.reporter.long_task(task_description, total=total_time) as task:
            while True:
                runtime = self.agentkit_runtime_client.get_runtime(runtime_types.GetRuntimeRequest(runtime_id=runtime_id))
    ...
    def _needs_runtime_update(self, runtime: runtime_types.GetRuntimeResponse, config: VeAgentkitRunnerConfig) -> Tuple[bool, str]:
        ...
        if runtime_envs != config.runtime_envs:
            ...
            update_reasons.append("Environment variables changed: " + "; ".join(env_changes))
        
        needs_update = len(update_reasons) > 0
        reason = " | ".join(update_reasons) if needs_update else "No configuration changes"
    ...
    def _update_existing_runtime(self, config: VeAgentkitRunnerConfig) -> DeployResult:
        ...
        try:
            ...
            if not needs_update:
                self.reporter.success("Runtime configuration is up-to-date, no update needed.")
                public_endpoint = self.get_public_endpoint_of_runtime(runtime)
                config.runtime_endpoint = public_endpoint
                config.runtime_apikey = runtime.authorizer_configuration.key_auth.api_key
            ...
            self.agentkit_runtime_client.update_runtime(runtime_types.UpdateRuntimeRequest(
                runtime_id=config.runtime_id,
                artifact_url=config.image_url,
            ...
            if not success:
                ...
                self._download_and_show_runtime_failed_logs(updated_runtime, config.runtime_id)
                return DeployResult(
                    success=False,
                    error=error,
                    error_code=ErrorCode.DEPLOY_FAILED,
            ...
            if updated_runtime.status == RUNTIME_STATUS_READY:
                self.reporter.success("Runtime updated directly to Ready status, no release step needed.")
            else:
                # Phase 2: Status is UnReleased, need to release the update
                self.reporter.info("Starting Runtime release...")
                self.agentkit_runtime_client.release_runtime(runtime_types.ReleaseRuntimeRequest(
                        runtime_id=config.runtime_id,
                    )
                )
                
                # Wait for release to complete
                self.reporter.info("Waiting for Runtime release to complete, status becoming Ready...")
                self.reporter.info("💡 Tip: Runtime is being released. Please wait patiently and do not interrupt.")
            ...
            self.reporter.info(f"Endpoint: {public_endpoint}")
            config.runtime_endpoint = public_endpoint
            config.runtime_apikey = updated_runtime.authorizer_configuration.key_auth.api_key
    ...
    @staticmethod
    def get_public_endpoint_of_runtime(runtime: runtime_types.GetRuntimeResponse) -> str:
