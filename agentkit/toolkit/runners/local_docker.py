...
class LocalDockerRunner(Runner):
    ...
    def deploy(self, config: LocalDockerRunnerConfig) -> DeployResult:
        ...
        try:
            ...
            error_msg = "Missing Docker dependencies, please install agentkit[docker] extras"
            logger.error(error_msg)
            self.reporter.error("Missing Docker dependencies")
            return DeployResult(
                success=False,
                error=error_msg,
        ...
        try:
            ...
            if success:
                ...
                self.reporter.success(f"Container deployed successfully: {docker_config.container_name}")
    ...
    def status(self, config: LocalDockerRunnerConfig) -> StatusResult:
        ...
        try:
            ...
            container_name_in_config = docker_config.container_name  # Must be saved in config
            image_name_in_config = docker_config.full_image_name  # Must be saved in config
            
            logger.info(f"Checking status with config: container_name={container_name_in_config}, image_name={image_name_in_config}")
            ...
            if docker_config.image_id:
                ...
                for img in images:
                    ...
                    if img_id.startswith(docker_config.image_id) or docker_config.image_id.startswith(img_id):
                        image_exists = True
                        image_info = img
                        actual_image_id = img_id
                        logger.info(f"Found image by ID: {img.get('tags', ['<none>'])[0]}")
                        break
            elif image_name_in_config:
                # Fallback: use explicitly saved image name from config
                logger.info(f"No image_id in config, using saved image name: {image_name_in_config}")
                images = self.docker_manager.list_images()
                for img in images:
                    tags = img.get('tags', [])
                    if image_name_in_config in tags or any(tag.startswith(image_name_in_config) for tag in tags):
                        image_exists = True
                        image_info = img
                        actual_image_id = img.get('id', '').replace('sha256:', '')
                        logger.info(f"Found image by name: {tags[0] if tags else '<none>'}")
                        break
            else:
            ...
            if docker_config.container_id:
                logger.info(f"Checking container by ID: {docker_config.container_id[:12]}")
                containers = self.docker_manager.list_containers(all_containers=True)
                for container in containers:
                    if container['id'].startswith(docker_config.container_id) or docker_config.container_id.startswith(container['id']):
                        container_exists = True
                        container_running = container['status'] == 'running'
                        container_info = container
                        logger.info(f"Found container by ID: {container['name']} ({container['status']})")
                        break
            elif container_name_in_config:
                # Fallback: use explicitly saved container name from config
                logger.info(f"No container_id in config, using saved container name: {container_name_in_config}")
                containers = self.docker_manager.list_containers(all_containers=True)
                for container in containers:
                    if container['name'] == container_name_in_config:
                        ...
                        container_running = container['status'] == 'running'
                        container_info = container
                        logger.info(f"Found container by name: {container['name']} ({container['status']})")
                        break
            else:
                # No container_id or container_name in config means container not deployed
                logger.info("No container_id or container_name in config, container not deployed")
            ...
            metadata = {
                ...
                'image': {
                    'name': image_info.get('tags', [''])[0] if image_info and image_info.get('tags') else None,
    ...
    def invoke(self, config: LocalDockerRunnerConfig, payload: Dict[str, Any], headers: Optional[Dict[str, str]] = None, stream: Optional[bool] = None) -> InvokeResult:
