...
from agentkit.toolkit.config import CommonConfig, AUTO_CREATE_VE, DEFAULT_WORKSPACE_NAME, DockerBuildConfig, DEFAULT_IMAGE_TAG
...
class VeCPCRBuilderConfig(AutoSerializableMixin):
    ...
    tos_bucket: str = field(default=AUTO_CREATE_VE, metadata={"description": "TOS bucket name", "render_template": True})
    tos_region: str = field(default="cn-beijing", metadata={"description": "TOS region"})
    tos_prefix: str = field(default="agentkit-builds", metadata={"description": "TOS path prefix"})
    ...
    cp_pipeline_name: str = field(default=AUTO_CREATE_VE, metadata={"description": "Code Pipeline name"})
    cp_pipeline_id: str = field(default="", metadata={"description": "Code Pipeline ID"})
    
    image_tag: str = field(default=DEFAULT_IMAGE_TAG, metadata={"description": "Image tag"})
    dockerfile_template: str = field(default="Dockerfile.j2", metadata={"description": "Dockerfile template"})
    build_timeout: int = field(default=3600, metadata={"description": "Build timeout in seconds"})
...
class VeCPCRBuilder(Builder):
    ...
    def build(self, config: VeCPCRBuilderConfig) -> BuildResult:
        ...
        try:
            ...
            resources['dockerfile_path'] = self._render_dockerfile(builder_config, docker_build_config)
            ...
            self.reporter.info(f"Uploaded to TOS: {resources['tos_url']}, bucket: {resources['tos_bucket']}")
            ...
            return BuildResult(
                ...
                metadata={
                    "resources": resources,
                    "tos_object_key": builder_config.tos_object_key if resources else None,
                    "cp_pipeline_id": resources.get('pipeline_id') if resources else None
                }
            )
    ...
    def check_artifact_exists(self, config: VeCPCRBuilderConfig) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.reporter.info(f"Checking image existence: {builder_config.image_url}")
    ...
    def remove_artifact(self, config: VeCPCRBuilderConfig) -> bool:
        ...
        try:
            ...
            if builder_config.tos_object_key:
                try:
                    from agentkit.toolkit.volcengine.services.tos_service import TOSService, TOSServiceConfig
            ...
            if builder_config.image_url:
                try:
                    self.reporter.warning(f"Note: CR image deletion not implemented, image preserved: {builder_config.image_url}")
            ...
            if builder_config.cp_pipeline_id:
                try:
                    self.reporter.warning(f"Note: Pipeline cleanup not implemented, Pipeline ID: {builder_config.cp_pipeline_id}")
    ...
    def _render_dockerfile(self, config: VeCPCRBuilderConfig, docker_build_config: Optional[DockerBuildConfig] = None) -> str:
        ...
        try:
            ...
            if docker_build_config:
                ...
                if docker_build_config.base_image:
                    if language == "Golang" and isinstance(docker_build_config.base_image, dict):
            ...
            if language == "Python":
                template_dir = os.path.abspath(os.path.join(
                    os.path.dirname(__file__), "..", "resources", "templates", "python"
                ))
                ...
                template_dir = os.path.abspath(os.path.join(
                    os.path.dirname(__file__), "..", "resources", "templates", "golang"
                ))
                entry_on_disk = (self.workdir / common_config.entry_point).resolve()
                if not entry_on_disk.exists():
                    raise Exception(f"Project path not found: {common_config.entry_point}")
                ...
                if entry_on_disk.is_file() and entry_on_disk.suffix == ".sh":
                    ...
                    try:
                        src_inside_proj = src_res.relative_to(proj_res) == src_res.relative_to(proj_res)
                    except Exception:
                        src_inside_proj = False
                    ...
                    entry_relative_path = str((Path("src") / project_root.name / entry_on_disk.name).as_posix())
                    binary_name = (common_config.agent_name or project_root.name)
                else:
                    # Directory-based build: copy project directory to src/<project>
                    project_root = entry_on_disk if entry_on_disk.is_dir() else entry_on_disk.parent
                    target_subdir = src_dest / project_root.name
                    proj_res = project_root.resolve()
                    src_res = src_dest.resolve()
    ...
    def _upload_to_tos(self, archive_path: str, config: VeCPCRBuilderConfig) -> str:
        ...
        try:
            ...
            if not tos_service.bucket_exists():
                ...
                self.reporter.warning(f"TOS bucket does not exist, creating: {bucket_name}")
                ...
                while time.time() - start_time < timeout:
                    if tos_service.bucket_exists():
                        self.reporter.success(f"TOS bucket verification successful: {bucket_name}")
                        break
                    else:
                        self.reporter.info(f"Waiting for TOS bucket to be ready... ({time.time() - start_time:.1f}s)")
                ...
                if config.tos_bucket and config.tos_bucket != AUTO_CREATE_VE:
                    self.reporter.info(f"Note: Your configured bucket '{config.tos_bucket}' did not exist, auto-created new bucket '{bucket_name}'")
            else:
    ...
    def _prepare_cr_resources(self, config: VeCPCRBuilderConfig) -> CRServiceConfig:
        ...
        try:
            ...
            from agentkit.toolkit.volcengine.services import CRService, DefaultCRConfigCallback
    ...
    def _prepare_pipeline_resources(self, config: VeCPCRBuilderConfig, tos_url: str, cr_config: CRServiceConfig) -> str:
        ...
        try:
            ...
            if region != 'cn-beijing':
                self.reporter.error("Error: Code Pipeline only supported in cn-beijing region")
                return False
            ...
            if not cp_client.workspace_exists_by_name(workspace_name):
                ...
                self.reporter.warning(f"Workspace '{workspace_name}' does not exist, creating...")
                ...
                if result.get("Items") and len(result["Items"]) > 0:
                    ...
                    logger.info(f"Using existing workspace: {workspace_name} (ID: {workspace_id})")
            ...
            if config.cp_pipeline_id and config.cp_pipeline_id != AUTO_CREATE_VE:
                try:
                    ...
                    if result.get("Items") and len(result["Items"]) > 0:
                        ...
                        logger.info(f"Reusing pipeline by ID: {found_pipeline_name} (ID: {config.cp_pipeline_id})")
                        self.reporter.success(f"Reusing pipeline by ID: {found_pipeline_name}")
                        ...
                        logger.warning(f"Configured Pipeline ID '{config.cp_pipeline_id}' does not exist, will create new pipeline")
                        self.reporter.warning("Configured Pipeline ID does not exist, will create new pipeline")
                        
                except Exception as e:
                    if "does not match" in str(e):
                        raise  # Name-ID mismatch, propagate exception
                    logger.warning(f"Pipeline lookup by ID failed: {str(e)}, will create new pipeline")
            ...
            # Case 2: If only pipeline name is configured (not AUTO_CREATE_VE), lookup by name
                ...
                try:
                    ...
                    if existing_pipelines.get("Items") and len(existing_pipelines["Items"]) > 0:
                        ...
                        logger.info(f"Reusing pipeline by name: {found_name} (ID: {pipeline_id})")
                        ...
                        logger.warning(f"Configured pipeline name '{config.cp_pipeline_name}' does not exist, will create new pipeline")
                        self.reporter.warning("Configured pipeline name does not exist, will create new pipeline")
                except Exception as e:
                    logger.warning(f"Pipeline lookup by name failed: {str(e)}, will create new pipeline")
            ...
            self.reporter.success(f"Pipeline created successfully: {pipeline_name} (ID: {pipeline_id})")
    ...
    def _execute_build(self, pipeline_id: str, config: VeCPCRBuilderConfig) -> str:
        ...
        try:
            ...
            if not hasattr(self, '_cp_client') or not hasattr(self, '_workspace_id'):
                raise Exception("Pipeline client not initialized, please call _prepare_pipeline_resources first")
            ...
            with self.reporter.long_task("Waiting for build completion...", total=max_wait_time) as task:
                last_status = None
