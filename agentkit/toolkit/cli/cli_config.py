...
def config_command(
    ...
    dry_run: bool = typer.Option(False, "--dry-run", help="Preview config changes without saving"),
    show: bool = typer.Option(False, "--show", "-s", help="Display current configuration"),
    
    # Global config support
    global_config: bool = typer.Option(False, "--global", "-g", help="Operate on global config (~/.agentkit/config.yaml)"),
    set_field: Optional[str] = typer.Option(None, "--set", help="Set field (format: key=value, e.g., cr.instance_name=my-cr)"),
    init_global: bool = typer.Option(False, "--init", help="Initialize global config file (create template)"),
    
    # CommonConfig parameters
    agent_name: Optional[str] = typer.Option(None, "--agent_name", help="Agent application name"),
    entry_point: Optional[str] = typer.Option(None, "--entry_point", help="Entry point file (e.g., agent.py)"),
    description: Optional[str] = typer.Option(None, "--description", help="Application description"),
    language: Optional[str] = typer.Option(None, "--language", help="Programming language: Python/Golang"),
    language_version: Optional[str] = typer.Option(None, "--language_version", help="Language version (e.g., Python: 3.10-3.13, Go: 1.x)"),
    python_version: Optional[str] = typer.Option(None, "--python_version", help="[Deprecated] Python version, use --language_version instead"),
    dependencies_file: Optional[str] = typer.Option(None, "--dependencies_file", help="Dependencies file (e.g., requirements.txt)"),
    launch_type: Optional[str] = typer.Option(None, "--launch_type", help="Deployment mode: local/hybrid/cloud"),
    
    # Application-level environment variables (CommonConfig)
    runtime_envs: Optional[List[str]] = typer.Option(None, "--runtime_envs", "-e", help="App-level env vars KEY=VALUE, shared across all deployment modes (can be used multiple times)"),
    
    # Strategy-level environment variables
    strategy_runtime_envs: Optional[List[str]] = typer.Option(None, "--strategy-runtime-envs", help="Strategy-level env vars KEY=VALUE, used only for current deployment mode (can be used multiple times)"),
    
    # Hybrid/Cloud Strategy parameters
    region: Optional[str] = typer.Option(None, "--region", help="Region (e.g., cn-beijing)"),
    tos_bucket: Optional[str] = typer.Option(None, "--tos_bucket", help="TOS bucket name"),
    image_tag: Optional[str] = typer.Option(None, "--image_tag", help="Image tag (e.g., 0.0.1)"),
    cr_instance_name: Optional[str] = typer.Option(None, "--cr_instance_name", "--ve_cr_instance_name", help="CR instance name"),
    cr_namespace_name: Optional[str] = typer.Option(None, "--cr_namespace_name", "--ve_cr_namespace_name", help="CR namespace"),
    cr_repo_name: Optional[str] = typer.Option(None, "--cr_repo_name", "--ve_cr_repo_name", help="CR repository name"),
    
    # Runtime configuration parameters
    runtime_name: Optional[str] = typer.Option(None, "--runtime_name", "--ve_runtime_name", help="Runtime instance name"),
    runtime_role_name: Optional[str] = typer.Option(None, "--runtime_role_name", "--ve_runtime_role_name", help="Runtime authorization role name"),
    runtime_apikey_name: Optional[str] = typer.Option(None, "--runtime_apikey_name", "--ve_runtime_apikey_name", help="Runtime API key secret name"),
):
...
def _interactive_config(config_file: Optional[str] = None):
    ...
    from agentkit.toolkit.cli.interactive_config import create_common_config_interactively
    common_config = create_common_config_interactively(config.get_common_config().to_dict())
...
def _handle_global_config(show: bool, set_field: Optional[str], init_global: bool):
    ...
    if init_global:
        ...
        if not global_config_exists():
            ...
            console.print("\n[bold]Usage:[/bold]")
            console.print("  • View config:  [yellow]agentkit config --global --show[/yellow]")
            console.print("  • Set field:    [yellow]agentkit config --global --set <field>=<value>[/yellow]")
            console.print("  • Edit file:    [yellow]vim ~/.agentkit/config.yaml[/yellow]")
...
def _init_global_config():
    ...
    if global_config_exists():
        console.print(f"[yellow]⚠️  Global config file already exists: {config_path}[/yellow]")
...
def _show_global_config():
    ...
    if not global_config_exists():
        console.print("[yellow]⚠️  Global config file does not exist: ~/.agentkit/config.yaml[/yellow]")
        console.print("\nQuick create:")
        console.print("  agentkit config --global --init")
        return
    ...
    console.print("\n[bold]🗂️  TOS Configuration[/bold]")
    console.print(f"  Bucket:     [yellow]{config.tos.bucket or '[dim](not set)[/dim]'}[/yellow]")
