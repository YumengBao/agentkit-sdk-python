...
class ConfigUpdates:
    ...
    def __repr__(self) -> str:
        ...
        return f"ConfigUpdates({len(self.updates)} updates: {list(self.updates.keys())})"
...
class InvokeResult:
    ...
    def stream(self):
        ...
        if not self.is_streaming:
            raise ValueError("This is not a streaming response. Check is_streaming before calling stream().")
        
        # Check if response is iterable (generator, list, etc.) but not string/bytes/dict
        if hasattr(self.response, '__iter__') and not isinstance(self.response, (str, bytes, dict)):
