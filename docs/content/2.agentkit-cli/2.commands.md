...
| 命令 | 用途 | 常用场景 |
|------|------|---------|
| `init` | 创建新项目 | 开始开发新 Agent |
| `config` | 配置应用 | 修改部署参数 |
| `build` | 构建镜像 | 打包 Agent 代码 |
| `deploy` | 部署应用 | 发布到运行环境 |
| `launch` | 构建+部署 | 快速上线 |
| `invoke` | 测试调用 | 验证 Agent 功能 |
| `status` | 查看状态 | 检查运行情况 |
| `destroy` | 清理资源 | 停止并删除实例 |
...
- 例如：`./my_agent.py` 或 `../agents/weather_agent.py`
- 文件必须包含 `Agent(...)` 定义
...
| 序号 | 模板名称 | 适用场景 | 包含内容 |
|------|---------|---------|---------|
| 1 | **basic** | 快速上手 | 最简单的 Agent 应用，快速上手 |
| 2 | **basic_stream** | 流式输出 | 支持流式输出的 Agent 应用 |
...
- **requirements.txt** - Python 依赖包列表（基础依赖：`veadk-python` 和 `veadk-python[extensions]`，某些模板包含额外依赖）
...
docker_build:
  base_image:
    builder: "golang:1.23-alpine"  # 编译阶段镜像
